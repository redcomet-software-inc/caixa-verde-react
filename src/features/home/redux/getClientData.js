...
export function getClientData(args = {}) {
  return (dispatch) => { // optionally you can have getState as the second argument
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      const doRequest = getClientInfo();
      doRequest.then(
        (res) => {
          dispatch({
            type: HOME_GET_CLIENT_DATA_SUCCESS,
            data: res,
          });
          resolve(res);
        },
        ...
        (err) => {
          dispatch({
            type: HOME_GET_CLIENT_DATA_FAILURE,
            data: { error: err },
          });
          reject(err);
        },
...
export function dismissGetClientDataError() {
  return {
...
export function reducer(state, action) {
  switch (action.type) {
    case HOME_GET_CLIENT_DATA_BEGIN:
      // Just after a request is sent
      return {
        ...state,
        getClientDataPending: true,
        getClientDataError: null,
      };

    case HOME_GET_CLIENT_DATA_SUCCESS:
    ...
    case HOME_GET_CLIENT_DATA_FAILURE:
      // The request is failed
      return {
        ...state,
        getClientDataPending: false,
        getClientDataError: action.data.error,
      };

    case HOME_GET_CLIENT_DATA_DISMISS_ERROR:
