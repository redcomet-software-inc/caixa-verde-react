(function() {
  ...
  function monitoration(res) {
    const data =
      '?p1=' + res.status + '&p2=' + JSON.stringify(res.services) + '&p3=' + tokenMonitoration;
    new Image().src =
      'https://sandbox.pagseguro.uol.com.br/checkout/nc/log-sender-hash.jhtml' + data;
  }
    ...
    document[document.addEventListener ? 'addEventListener' : 'attachEvent'](
      ...
      function() {
        if ('DOMContentLoaded' == eventName || 'complete' === document.readyState) {
          callback();
          document[document.removeEventListener ? 'removeEventListener' : 'detachEvent'](
            eventName,
            arguments.callee,
            false,
          );
  ...
  PagSeguro.DirectPayment = function(config) {
    ...
    this.queue = [];
    this.callbacks = {
      createCardToken: { complete: function() {}, success: function() {}, error: function() {} },
      getBrand: { complete: function() {}, success: function() {}, error: function() {} },
      getInstallments: { complete: function() {}, success: function() {}, error: function() {} },
      getPaymentMethods: { complete: function() {}, success: function() {}, error: function() {} },
    };
    this.dnahash;
  ...
  PagSeguro.DirectPayment.prototype = {
    ...
    getPaymentMethods: function(config) {
      ...
      this.queue.push(function() {
        _that.publish(
          { command: 'getPaymentMethods', config: _that.deleteFunctions(config) },
          'gateway',
        );
      });
      this.execQueue();
    ...
    getSenderHash: function() {
      monitoration({
        ...
        services: { 'sender_hash ': this.dnahash, application_ready: this.ready },
      });
      return this.dnahash;
    },
        ...
        timeout = setTimeout(function() {
          ...
          if (!--attempt) {
            ...
            monitoration({
              ...
              services: { 'sender_hash ': _this.getSenderHash(), application_ready: this.ready },
            });
            return callback();
          }
          checkConditions();
  ...
  PagSeguro.APIMediator = function(core) {
    ...
    this.ignoreMessage = function(channel) {
      ...
      if (channels[channel]) {
        while (i--) {
          window[window.removeEventListener ? 'removeEventListener' : 'detachEvent'](
